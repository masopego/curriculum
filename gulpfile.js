...
gulp.task('documents', (done) => {
  gulp
    .src(config.documents.src)
    .pipe(
      plumber({ errorHandler: notify.onError('Error: <%= error.message %>') })
    )
...
gulp.task('documents-dist', (done) => {
  gulp
    .src(config.documents.src)
    .pipe(
      plumber({ errorHandler: notify.onError('Error: <%= error.message %>') })
    )
    .pipe(gulp.dest(config.documents.dist));
  done();
});
// main tasks
...
gulp.task(
  'default',
  gulp.series(
    ['clean', 'api', 'html', 'css', 'js', 'images', 'documents'],
    (done) => {
